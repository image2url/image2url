...
export class Image2UrlClient {
  ...
  async uploadBase64(base64: string, options?: UploadOptions): Promise<UploadResult> {
  ...
  async uploadBuffer(data: Buffer | ArrayBuffer | Uint8Array, options?: UploadOptions): Promise<UploadResult> {
    ...
    const contentType = ensureImageContentType(
      options?.contentType ?? guessContentType(options?.originalName) ?? 'image/jpeg',
    );
  ...
  private buildKey(key: string | undefined, originalName: string | undefined, contentType: string): string {
...
export function createClientFromEnv(env: NodeJS.ProcessEnv = process.env): Image2UrlClient {
  return new Image2UrlClient({
    accountId: requireValue(
      firstEnv(env, ['R2_ACCOUNT_ID', 'IMGTOURL_ACCOUNT_ID', 'IMAGETOURL_ACCOUNT_ID', 'IMAGE2URL_ACCOUNT_ID']),
      'R2_ACCOUNT_ID',
    ),
    accessKeyId: requireValue(
      firstEnv(env, ['R2_ACCESS_KEY_ID', 'IMGTOURL_ACCESS_KEY_ID', 'IMAGETOURL_ACCESS_KEY_ID', 'IMAGE2URL_ACCESS_KEY_ID']),
      'R2_ACCESS_KEY_ID',
    ),
    ...
    bucket: requireValue(
      firstEnv(env, ['R2_BUCKET_NAME', 'IMGTOURL_BUCKET', 'IMAGETOURL_BUCKET', 'IMAGE2URL_BUCKET']),
      'R2_BUCKET_NAME',
    ),
    publicUrl: requireValue(
      firstEnv(env, ['R2_PUBLIC_URL', 'IMGTOURL_PUBLIC_URL', 'IMAGETOURL_PUBLIC_URL', 'IMAGE2URL_PUBLIC_URL']),
      'R2_PUBLIC_URL',
    ),
...
function ensureImageContentType(contentType: string): string {
  if (!contentType) {
...
function guessContentType(originalName?: string): string | undefined {
  if (!originalName) return undefined;
  const ext = path.extname(originalName).toLowerCase();
  return MIME_BY_EXT[ext];
}

function guessExtension(originalName: string | undefined, contentType: string): string | undefined {
...
function buildMetadata(
  ...
  const result: Record<string, string> = metadata ? { ...metadata } : {};
  if (originalName) {
