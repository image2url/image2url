...
async function main() {
    ...
    try {
        const client = new Image2UrlClient({
            accountId: requireArg(args, ['account', 'a'], ['R2_ACCOUNT_ID', 'IMGTOURL_ACCOUNT_ID', 'IMAGETOURL_ACCOUNT_ID', 'IMAGE2URL_ACCOUNT_ID'], 'account id'),
            accessKeyId: requireArg(args, ['access-key'], ['R2_ACCESS_KEY_ID', 'IMGTOURL_ACCESS_KEY_ID', 'IMAGETOURL_ACCESS_KEY_ID', 'IMAGE2URL_ACCESS_KEY_ID'], 'access key'),
            secretAccessKey: requireArg(args, ['secret-key'], [
                'R2_SECRET_ACCESS_KEY',
                'IMGTOURL_SECRET_ACCESS_KEY',
                'IMAGETOURL_SECRET_ACCESS_KEY',
                'IMAGE2URL_SECRET_ACCESS_KEY',
            ], 'secret key'),
            bucket: requireArg(args, ['bucket', 'b'], ['R2_BUCKET_NAME', 'IMGTOURL_BUCKET', 'IMAGETOURL_BUCKET', 'IMAGE2URL_BUCKET'], 'bucket name'),
            publicUrl: requireArg(args, ['public-url'], ['R2_PUBLIC_URL', 'IMGTOURL_PUBLIC_URL', 'IMAGETOURL_PUBLIC_URL', 'IMAGE2URL_PUBLIC_URL'], 'public URL'),
...
function readString(args, keys, envKeys = []) {
...
function readNumber(args, keys, envKeys = []) {
...
function requireArg(args, keys, envKeys, label) {
    const value = readString(args, keys, envKeys);
    if (!value) {
        throw new Error(`Missing ${label}. Provide --${keys[0]} or set ${envKeys.join(' / ')}.`);
    }
    return value;
}
